...
class MultimodalNER(nn.Module):
    def __init__(self, text_encoder_path="chinese-roberta-www-ext",
                 image_encoder_path="clip-patch32",
                 num_labels=9, hidden_dim=256):
        ...
        self.image_hidden_size = self.clip.vision_model.config.hidden_size  # Usually 768
    ...
    def forward(self, input_ids, attention_mask, image_tensor, labels=None):
        ...
        input_ids: [B, T]
        attention_mask: [B, T]
        image_tensor: [B, 3, 224, 224]  # Already preprocessed by CLIPProcessor
        labels: [B, T] (optional)
        """
        # 1. 文本编码
        ...
        # 2. 图像编码
        with torch.no_grad():
            image_feat = self.clip.get_image_features(pixel_values=image_tensor)  # [B, D]
        image_feat = self.clip_proj(image_feat)  # [B, H]
        image_feat = image_feat.unsqueeze(1).repeat(1, text_feat.size(1), 1)  # [B, T, H]

        # 3. 拼接图文特征
        fused_feat = torch.cat([text_feat, image_feat], dim=-1)  # [B, T, 2H]
...
if __name__ == "__main__":
    ...
    roberta_name = "chinese-roberta-www-ext"
    clip_name = "clip-patch32"

    # 🧠 初始化模型和tokenizer
    tokenizer = BertPreTrainedModel.from_pretrained(roberta_name)

    clip_model = CLIPModel.from_pretrained(clip_name).to(device)
    clip_processor = CLIPProcessor.from_pretrained(clip_name)

    # 👀 测试数据
    test_text = "Trump Officials Offer Mixed Signals on Trade Deal With China"
