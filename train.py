...
def train(config):
    ...
    swanlab_name = "{0}_train-{1}_{2}_{3}".format(datetime.now().strftime('%Y-%m-%d'), config.dataset_name, str(config.model), config.ex_name)
    print("train pth save name and swanlab name is", swanlab_name)
    # 初始化实验ex_project
    swanlab.init(
        project=config.ex_project,
        name="{0}".format(swanlab_name),
        config={
    ...
    save_dir = os.path.join(script_dir, "save_models", "{0}".format(swanlab_name))
    ...
    DATA_PATH = {
        ...
        "NewsMKG": {
            ...
            'train': 'data/NewsMKG/train.txt',
            'valid': 'data/NewsMKG/valid.txt',
            'test': 'data/NewsMKG/test.txt',
        }
    }
    # image data
    IMG_PATH = {
    ...
    train_dataset = MMPNERDataset(
        ...
        sample_ratio=1.0, mode='train', return_span=False
    )
